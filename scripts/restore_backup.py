...
def main() -> int:
    parser = argparse.ArgumentParser(description="Restore backup Face Recognition System")
    parser.add_argument(
        "backup_path",
        type=Path,
        help="Backup path (ex. backups/20260521_030000)",
    )
    parser.add_argument(
        "--only",
        choices=["postgres", "qdrant", "minio"],
        action="append",
        help="Restore only the specified component(s) (repeatable).",
    )
    ...
    if not backup_dir.is_dir():
        print(f"Directory not found: {backup_dir}", file=sys.stderr)
        return 1
